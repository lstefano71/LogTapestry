...
namespace LogTapestry.Core
{
    public class RegexLogParser : ILogParser
    {
        ...
        private readonly Regex _startOfEntryRegex = new(".");
        ...
        public RegexLogParser(PluginSettings settings, string sourceFile)
        {
            ...
            if (settings.Config?.StartOfEntryRegex != null)
            {
                _startOfEntryRegex = new Regex(settings.Config.StartOfEntryRegex, RegexOptions.Compiled);
            }
        }
        ...
        {
            _multiLineBuffer.Clear().Append(line);
